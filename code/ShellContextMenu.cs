...
namespace LightFileExplorer
{
    ...
    internal static class ShellHelper
    {
        public static uint HiWord(IntPtr ptr)
        {
            ...
            if ((param32 & 0x80000000) == 0x80000000) return (param32 >> 16); else return (param32 >> 16) & 0xffff;
        }
    ...
    internal class ShellContextMenu : NativeWindow
    {
        ...
        public void ShowContextMenu(string path, IList<string> items, Point point)
        {
            ReleaseAll();

            _arrPIDLs = GetPIDLs(path, items);
        ...
        protected IntPtr[] GetPIDLs(string path, IList<string> items)
        {
            IShellFolder oParentFolder = GetParentFolder(path);
            ...
            IntPtr[] arrPIDLs = new IntPtr[items.Count];
            ...
            foreach (var item in items)
            {
                ...
                int nResult = oParentFolder.ParseDisplayName(IntPtr.Zero, IntPtr.Zero, item, ref pchEaten, out pPIDL, ref pdwAttributes);
        ...
        {
            if (_oContextMenu != null && m.Msg == (int)WM.MENUSELECT && ((int)ShellHelper.HiWord(m.WParam) & (int)MFT.SEPARATOR) == 0 && ((int)ShellHelper.HiWord(m.WParam) & (int)MFT.POPUP) == 0)
            {
                return;
            }

            if (_oContextMenu2 != null && (m.Msg == (int)WM.INITMENUPOPUP || m.Msg == (int)WM.MEASUREITEM || m.Msg == (int)WM.DRAWITEM))
            {
        ...
        private static extern bool AppendMenu(IntPtr hMenu, MFT uFlags, uint uIDNewItem, [MarshalAs(UnmanagedType.LPTStr)] string lpNewItem);
        ...
        [DllImport("User32", SetLastError = true, CharSet = CharSet.Auto)]
        private static extern bool GetMenuInfo(IntPtr hMenu, ref MENUINFO lpcmi);
        ...
        [DllImport("ShlwApi", EntryPoint = "StrRetToBuf", ExactSpelling = false, CharSet = CharSet.Auto, SetLastError = true)]
        ...
        {
            int nResult = oParentFolder.GetUIObjectOf(IntPtr.Zero, (uint)arrPIDLs.Length, arrPIDLs, ref IID_IContextMenu, IntPtr.Zero, out ctxMenuPtr);
        ...
        private void InvokeCommand(IContextMenu oContextMenu, uint nCmd, string strFolder, Point pointInvoke)
        {
            ...
            invoke.fMask = CMIC.UNICODE | CMIC.PTINVOKE | ((Control.ModifierKeys & Keys.Control) != 0 ? CMIC.CONTROL_DOWN : 0) | ((Control.ModifierKeys & Keys.Shift) != 0 ? CMIC.SHIFT_DOWN : 0);
        ...
        {
            IntPtr pMenu = IntPtr.Zero, iContextMenuPtr = IntPtr.Zero, iContextMenuPtr2 = IntPtr.Zero, iContextMenuPtr3 = IntPtr.Zero;
            ...
            try
            {
                ...
                int nResult = _oContextMenu.QueryContextMenu(pMenu, 0, CMD_FIRST, CMD_LAST, CMF.NORMAL);
